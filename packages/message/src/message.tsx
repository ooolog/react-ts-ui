import { optionfnProps, messagesProps } from '@t-ui/types/types';
...
export default function Message(options: messagesProps = {}) {
    ...
    const classes = classNames('i-message', className, {
        [`i-message-${type}`]: type
    })
    ...
    let _options: optionfnProps = {
      ...
      `;
        ...
        dom() {
            ...
            let len = document.querySelectorAll('.i-message').length;
            this.doms = dom;
            if (!showClose) {
                setTimeout(() => {
                    this.remove(dom)
        ...
        closeAll() {
            let domsAll = document.querySelectorAll('.i-message');
            if (domsAll) {
